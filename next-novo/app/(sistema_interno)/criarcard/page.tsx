...
export default function CreateCardPage() {
  ...
  function validarFormulario() {
    ...
    atributos.forEach((atributo) => {
      ...
      if (valor < 1 || valor > 20) {
        novosErros[atributo] = "Valor deve ser entre 1 e 20.";
      }
    });
    ...
    for (const skill of skillsSelecionadas) {
      if (skill.valor < 0 || skill.valor > 20) {
        novosErros.skills = "Os valores das skills devem estar entre 0 e 20.";
        break;
      }
    }
    ...
    if (Object.keys(novosErros).length > 0) {
      alert(
        "Preencha todos os campos obrigatórios e selecione ao menos uma Skill!",
      );
  ...
  async function salvarNPC(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // ZERA OS BUFFERS / CAMPOS APÓS SALVAR
      setNpc({
        nome: "",
        classe: "",
        raca: "",
  ...
  return (
    <main className="cardPage">
      ...
      <form className="cardForm" onSubmit={salvarNPC} noValidate>
              ...
              {skills.map((skill, index) => (
                <div key={skill.nome} className="skillLinha">
                  ...
                  <span className="skillNome">
                    {skill.nome}
                    <span className="skillAtributo">({skill.atributo})</span>
                  </span>
                </div>
              ))}
            </div>
        ...
        <div className="attributesGrid">
          <div className="attribute">
            ...
            />
            {erros.forca && <span className="erro">{erros.forca}</span>}
          </div>
          ...
          <div className="attribute">
            ...
            />
            {erros.destreza && <span className="erro">{erros.destreza}</span>}
          </div>
          ...
          <div className="attribute">
            ...
            />
            {erros.sabedoria && <span className="erro">{erros.sabedoria}</span>}
          </div>
          ...
          <div className="attribute">
            ...
            />
            {erros.carisma && <span className="erro">{erros.carisma}</span>}
          </div>
        </div>
